  ...
  if (submitted) {
    ...
    return (
      <div className="max-w-3xl">
        ...
        <p className="mb-6 text-gray-600">หมายเลขคำสั่งซื้อ: <span className="font-semibold">{orderId}</span></p>
        ...
        <div className="mb-6 rounded-2xl border bg-gray-950 p-4">
          ...
          <div className="text-sm text-gray-700">
            ...
            <div>{submitted.district}, {submitted.province} {submitted.postalCode}</div>
            {submitted.phone && <div>โทร: {submitted.phone}</div>}
            <div>อีเมล: {submitted.email}</div>
            {submitted.notes && <div className="mt-2">หมายเหตุ: {submitted.notes}</div>}
          </div>
        </div>
    ...
    <div className="grid gap-8 md:grid-cols-3">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="md:col-span-2 space-y-4">
        ...
        <div>
          ...
          <input {...register("fullName")} className="mt-1 w-full rounded-xl border px-3 py-2" />
          {errors.fullName && <p className="mt-1 text-sm text-red-600">{errors.fullName.message}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium">อีเมล</label>
          <input {...register("email")} className="mt-1 w-full rounded-xl border px-3 py-2" />
          {errors.email && <p className="mt-1 text-sm text-red-600">{errors.email.message}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium">ที่อยู่</label>
          <textarea {...register("address")} rows={3} className="mt-1 w-full rounded-xl border px-3 py-2" />
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <input {...register("district")} className="mt-1 w-full rounded-xl border px-3 py-2" />
          ...
          <div>
            ...
            <input {...register("province")} className="mt-1 w-full rounded-xl border px-3 py-2" />
          ...
          <div>
            ...
            <input {...register("postalCode")} className="mt-1 w-full rounded-xl border px-3 py-2" />
            {errors.postalCode && <p className="mt-1 text-sm text-red-600">{errors.postalCode.message}</p>}
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium">เบอร์โทร (ไม่บังคับ)</label>
          <input {...register("phone")} className="mt-1 w-full rounded-xl border px-3 py-2" />
          {errors.phone && <p className="mt-1 text-sm text-red-600">{errors.phone.message}</p>}
        </div>

        <div>
          <label className="block text-sm font-medium">หมายเหตุ</label>
          <textarea {...register("notes")} rows={2} className="mt-1 w-full rounded-xl border px-3 py-2" />
      ...
      <aside className="md:col-span-1">
        <div className="rounded-2xl border bg-black p-4">
          ...
          <div className="mt-3 border-t pt-3 text-right">
            ...
            <div className="text-lg font-semibold">{formatBaht(totalPrice)}</div>
          </div>
        </div>
      </aside>
    </div>
